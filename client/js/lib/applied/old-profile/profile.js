...
const profile_data = [
   ...
   { x: 1, y: 0, texture: 'uv_grid'},

   { x: 0, y: -2, r: 2, texture: 'bump', },
   { x: -2, y: -2, texture: 'uv_grid' },
   { x: -3, y: -1, texture: 'uv_grid' },
   { x: -3, y: 1, texture: 'uv_grid' },
];
   ...
   for (let i = 0; i < len; i++) {
      ...
      if (p0.r) {
         ...
         if (p0.texture) {
            ...
            const start_i = end_i - 2 - (arc_points.length / 3);
            textures_indices.push({
               texture_name: p0.texture,
               start_i,
               end_i,
            });
         };
